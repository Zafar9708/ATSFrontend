...
const AdminDashboard = () => {
  ...
  const emptyRecruiterForm = {
    ...
    username: '', location: '', linkedIn: '',
    designation: '', department: '', employmentType: '', experience: '',
    specialisation: '', secondarySkill: '', joiningDate: '',
  ...
  const validateRecruiterForm = () => {
    const errors = {};
    if (!recruiterForm.firstName.trim())   errors.firstName   = 'First name is required';
    if (!recruiterForm.lastName.trim())    errors.lastName    = 'Last name is required';
    if (!recruiterForm.email.trim())       errors.email       = 'Email is required';
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(recruiterForm.email)) errors.email = 'Invalid email address';
    if (!recruiterForm.phoneNumber.trim()) errors.phoneNumber = 'Phone number is required';
    if (!recruiterForm.username.trim())    errors.username    = 'Username is required';
    if (!editingId) {
      if (!recruiterForm.password)               errors.password        = 'Password is required';
      else if (recruiterForm.password.length < 8) errors.password       = 'Minimum 8 characters';
      if (recruiterForm.password !== recruiterForm.confirmPassword) errors.confirmPassword = 'Passwords do not match';
    } else if (recruiterForm.password && recruiterForm.password !== recruiterForm.confirmPassword) {
  ...
  const handleSaveRecruiter = async () => {
    ...
    try {
      const fd = new FormData();
      const fields = ['firstName','lastName','email','phoneNumber','username','location',
        'linkedIn','designation','department','employmentType','experience',
        'specialisation','secondarySkill','joiningDate'];
      fields.forEach(key => { if (recruiterForm[key]) fd.append(key, recruiterForm[key]); });
      if (recruiterForm.password) fd.append('password', recruiterForm.password);
      if (recruiterForm.profilePicture instanceof File) fd.append('profilePicture', recruiterForm.profilePicture);
      if (recruiterForm.customNotes?.length) fd.append('customNotes', JSON.stringify(recruiterForm.customNotes));
      await adminService.addRecruiter(fd);
      setSavedRecruiterEmail(recruiterForm.email);
      setSaveSuccess(true);
      await fetchRecruiters();
    } catch (err) {
      showSnackbar(err.response?.data?.message || 'Error saving recruiter', 'error');
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box sx={{
        ...
        {/* ════════════════════════════════════════════════════════════════════
            ADD / EDIT RECRUITER DIALOG — uses RecruiterForm component
        ════════════════════════════════════════════════════════════════════ */}
        <Dialog
          open={openDialog}
          onClose={handleCloseDialog}
          ...
          PaperProps={{
            sx: {
              ...
              '&::-webkit-scrollbar':       { width: 6 },
              '&::-webkit-scrollbar-track': { background: '#f1f1f1' },
              '&::-webkit-scrollbar-thumb': { background: T.primaryMid, borderRadius: 3 },
            }
          }}
        >
          {saving && (
            <LinearProgress sx={{ height: 2.5, '& .MuiLinearProgress-bar': { background: '#93c5fd' } }} />
          )}
          ...
          {saveSuccess ? (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', py: { xs: 3, sm: 5 }, px: { xs: 2, sm: 4 } }}>
              ...
              <Typography sx={{ fontSize: { xs: 18, sm: 22 }, fontWeight: 900, color: T.text, mb: 0.75, letterSpacing: '-0.3px' }}>
                {editingId ? 'Recruiter Updated!' : 'Recruiter Added Successfully!'}
              </Typography>
              <Typography sx={{ fontSize: { xs: 12, sm: 13.5 }, color: T.textSub, mb: 3.5, fontWeight: 500, lineHeight: 1.6, maxWidth: 320 }}>
                The recruiter account has been {editingId ? 'updated' : 'created'} and is ready to use.
              </Typography>
              <Box sx={{
                display: 'flex', alignItems: 'flex-start', gap: 1.75, textAlign: 'left',
                background: `linear-gradient(135deg, ${T.primaryLight} 0%, #dbeafe 100%)`,
                border: `1.5px solid ${T.primaryMid}20`, borderRadius: '14px',
                px: { xs: 1.5, sm: 2.5 }, py: { xs: 1.5, sm: 2 }, maxWidth: 370, width: '100%',
