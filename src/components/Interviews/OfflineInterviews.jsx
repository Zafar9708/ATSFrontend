  ...
  useEffect(() => {
    const fetchInterviews = async () => {
      ...
      try {
        ...
        console.error('Error fetching offline interviews:', e);
        setError(e.response?.data?.message || 'Failed to load interviews. Please try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error) {
    return (
      <Box sx={{ 
        ...
        alignItems: 'center', minHeight: '50vh', textAlign: 'center', p: 3
      }}>
    ...
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1200, margin: '0 auto' }}>
      <Box sx={{ 
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Typography variant="h4" sx={{ fontWeight: 700 }}>Offline Interviews</Typography>
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Badge 
            ...
            color="secondary"
            sx={{ '& .MuiBadge-badge': { fontSize: '0.9rem', height: 26, minWidth: 26, borderRadius: 13, padding: '0 8px' } }}
          />
        </Box>
      </Box>
      ...
      {filteredInterviews.length === 0 ? (
        <Paper elevation={0} sx={{ 
          ...
          <Button
            variant="contained" color="secondary" size="large" startIcon={<CalendarIcon />}
              ...
              <CardContent>
                <Box sx={{ 
                  ...
                  <Box>
                    ...
                    {/* round — real API field */}
                    {interview.round && (
                      <Chip label={interview.round} size="small" sx={{ mt: 0.5, fontSize: 11, height: 22, fontWeight: 600 }} />
                    )}
                    {interview.jobId && (
                      <>
                ...
                <Box sx={{ 
                  display: 'flex', flexWrap: 'wrap', gap: 3, mb: 2,
                  ...
                  <Tooltip title="Location">
                    <Box>
                      ...
                      <Typography variant="body2">
                        {/* real API: address/building/floor — no city */}
                        {interview.location?.address || interview.location?.building || 'No location'}
                      </Typography>
                    </Box>
                  </Tooltip>
                </Box>
                  ...
                  <Box sx={{ mt: 2, pl: 1, pr: 1 }}>
                    ...
                    {interview.jobId && (
                      <>
                        ...
                        <Box sx={{
                          display: 'flex', flexDirection: 'column', gap: 1, mb: 3, p: 2,
                          borderRadius: 2, backgroundColor: theme.palette.background.default
                        }}>
                      ...
                      <>
                        ...
                        <List dense sx={{ mb: 3 }}>
                          {interview.location.address && (
                            <ListItem>
                              <ListItemIcon sx={{ minWidth: 36 }}><LocationIcon fontSize="small" /></ListItemIcon>
                              <ListItemText primary="Address" secondary={interview.location.address} secondaryTypographyProps={{ variant: 'body2' }} />
                            </ListItem>
                          )}
                          {interview.location.building && (
                            <ListItem>
                              <ListItemIcon sx={{ minWidth: 36 }}><LocationIcon fontSize="small" /></ListItemIcon>
                              <ListItemText primary="Building" secondary={interview.location.building} secondaryTypographyProps={{ variant: 'body2' }} />
                            </ListItem>
                          )}
                          {interview.location.floor && (
                            <ListItem>
                              ...
                              <ListItemText 
                                primary="Floor" 
                                secondary={[interview.location.floor, interview.location.room].filter(Boolean).join(', ')} 
                                secondaryTypographyProps={{ variant: 'body2' }} 
                              />
                            </ListItem>
                          )}
                          {(interview.location.city || interview.location.country) && (
                            <ListItem>
                              <ListItemIcon sx={{ minWidth: 36 }}><LocationIcon fontSize="small" /></ListItemIcon>
                              <ListItemText 
                                primary="City / Country" 
                                secondary={[interview.location.city, interview.location.country].filter(Boolean).join(', ')} 
                                secondaryTypographyProps={{ variant: 'body2' }} 
                              />
                            </ListItem>
                          )}
                    ...
                    {interview.notes && (
                      <>
                        ...
                        <Paper elevation={0} sx={{ 
                          ...
                          backgroundColor: theme.palette.background.default, whiteSpace: 'pre-wrap'
                        }}>
                          {interview.notes}
                        </Paper>
                      </>
                    ...
                    {interview.feedbackStatus && (
                      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 2 }}>
                        Feedback: {interview.feedbackStatus.submitted} / {interview.feedbackStatus.total} submitted
                      </Typography>
                    )}
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2 }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        <Button
                          size="small" variant="outlined" startIcon={<EmailIcon />}
                        ...
                        <Button
                          size="small" variant="contained" color="secondary" endIcon={<MoreIcon />}
