...
const CandidatesTab = () => {
    ...
    const stageCardData = [
        { stage: 'sourced',     label: 'Sourced',     count: stageCounts.all,        totalCount: stageCounts.all },
    ...
    const handleOpenBulkUpload  = () => setBulkUploadOpen(true);
    const handleCloseBulkUpload = () => setBulkUploadOpen(false);
    ...
    const handleCloseDetails = () => setOpenDetailsDialog(false);
    ...
    const handleCloseInterviewMenu = () => setInterviewAnchorEl(null);
    const handleCloseStageMenu    = () => setStageAnchorEl(null);
    const handleCloseRemarksMenu  = () => setRemarksAnchorEl(null);
    ...
    const handleBulkEmail = async () => setEmailDialogOpen(true);
    ...
    const handleRejectedFilterChange = (event) => setRejectedFilter(event.target.value);
    ...
    const handleOpenAnalysis = async (candidateId) => {
        ...
        try {
            const data = await candidateService.getResumeAnalysis(candidateId);
    ...
    const handleStageCardClick = (stage) => {
        setFilters({ ...filters, status: stage === 'sourced' ? '' : stage });
        setRejectedFilter('');
    };
    ...
    return (
        <Box sx={{ p: 1, marginLeft: 15, width:"85%" }}>
          ...
          <Box sx={{ mb: 2 }}>
            ...
            <Dialog open={bulkMoveDialogOpen} onClose={() => setBulkMoveDialogOpen(false)}>
                ...
                <DialogActions>
                    ...
                    <Button onClick={handleBulkStageMove} variant="contained" disabled={!newStage}>
                        Move Candidates
                    </Button>
                </DialogActions>
            </Dialog>
                ...
                <Box sx={{ mb: 2 }}>
                    <FormControl size="small" sx={{ minWidth: 200 }}>
                        ...
                        >
                            ...
                            {rejectionTypes && rejectionTypes.map(type => (
                    ...
                    <FormControl size="small" sx={{ minWidth: 150 }}>
                        ...
                        >
                            ...
                            <MenuItem value="move-to-sourced">
                                <ListItemIcon><StageIcon fontSize="small" /></ListItemIcon>
            ...
            {/* ── Interview Menu ───────────────────────────────────── */}
            <Menu anchorEl={interviewAnchorEl} open={Boolean(interviewAnchorEl)} onClose={handleCloseInterviewMenu}>
            ...
            {/* ── Stage Menu ───────────────────────────────────────── */}
            <Menu anchorEl={stageAnchorEl} open={Boolean(stageAnchorEl)} onClose={handleCloseStageMenu}>
                <MenuItem onClick={() => { setMoveDialogOpen(true); handleCloseStageMenu(); }}>
                    <ListItemText>Move to Another Stage</ListItemText>
                </MenuItem>
            </Menu>

            {/* ── Remarks Menu ─────────────────────────────────────── */}
            <Menu anchorEl={remarksAnchorEl} open={Boolean(remarksAnchorEl)} onClose={handleCloseRemarksMenu}>
                <MenuItem onClick={handleAddRemarks}>
                    <ListItemIcon><RemarksIcon fontSize="small" /></ListItemIcon>
            ...
            >
                <DialogTitle sx={{
                    ...
                    <IconButton edge="end" color="inherit" onClick={() => setAnalysisDialogOpen(false)} aria-label="close">
                        <CloseIcon />
                    </IconButton>
                </DialogTitle>
                    ...
                    <Table>
                        <TableHead>
                            <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                                ...
                                {["Name", "Status", "Experience", "Source", "Available to join", "Email", "Phone", "Actions"].map((label, index) => (
                                    <TableCell
                                        key={index}
                                        sx={{ fontWeight: 'bold', fontSize: '0.85rem', color: '#333', borderBottom: '2px solid #e0e0e0' }}
                                    >
                                        {label}
                                    </TableCell>
                                ))}
                            ...
                            {getFilteredCandidates().map((candidate) => (
                                ...
                                >
                                            ...
                                            <Box>
                                                {(() => {
                                                    try {
                                                        ...
                                                        const rejectionType = candidate?.rejectionType ? String(candidate.rejectionType) : null;
                                                        return (
                                                            <>
                                                        ...
                                                        return <Typography color="error">Error loading stage</Typography>;
                                                    }
                                                })()}
                                            </Box>
                                        </ErrorBoundary>
                                    ...
                                    <TableCell onClick={(e) => e.stopPropagation()}>
                                        <IconButton className="action-button" onClick={(e) => handleInterviewClick(e, candidate._id)}>
                                            <InterviewIcon />
                                        </IconButton>
                                        <IconButton className="action-button" onClick={(e) => handleStageClick(e, candidate._id)}>
                                            <StageIcon />
                                        </IconButton>
                                        <IconButton className="action-button" onClick={(e) => handleRemarksClick(e, candidate._id)}>
                                            <MoreIcon />
                                        </IconButton>
                                        <Button
                                            variant="outlined"
                    ...
                    {getFilteredCandidates().map((candidate) => (
                        ...
                        >
                            <CardContent sx={{ display: "flex", flexDirection: "column", gap: 3, padding: 3 }}>
                                {/* Header: Name, Avatar, Checkbox */}
                                <Box sx={{ display: "flex", alignItems: "center", gap: 3 }}>
                                    <Checkbox
                                        checked={selectedCandidates.includes(candidate._id)}
                                        onChange={(e) => { e.stopPropagation(); handleSelectCandidate(candidate._id); }}
                                        color="primary"
                                        sx={{ padding: 0 }}
                                    />
                                    <Avatar
                                        sx={{ bgcolor: "primary.main", fontSize: "1.4rem", fontWeight: "bold", width: 48, height: 48 }}
                                        onClick={() => handleNavigateToCandidate(candidate)}
                                    >
                                    ...
                                    <IconButton
                                        ...
                                        onClick={(e) => { e.stopPropagation(); handleRemarksClick(e, candidate._id); }}
                                    >
                                        <MoreIcon />
                                    </IconButton>
                                </Box>
                                ...
                                {getStageName(candidate.stage) === "Rejected" && candidate.rejectionType && (
                                    <Box sx={{ backgroundColor: '#ffeeee', p: 1, borderRadius: 1, borderLeft: '3px solid #f44336' }}>
                                        <Typography variant="body2" sx={{ fontWeight: 500 }}>{candidate.rejectionType}</Typography>
                                ...
                                {/* Action Buttons */}
                                <Box
                                    sx={{ display: "flex", justifyContent: "space-between", marginTop: 3, gap: 1 }}
                                    onClick={(e) => e.stopPropagation()}
                                >
                                    <Button
                                        variant="contained"
                                        startIcon={<AnalysisIcon />}
                                        onClick={(e) => { e.stopPropagation(); handleOpenAnalysis(candidate._id); }}
                                        disabled={!candidate.resume}
                                        sx={{
                                            flex: 1,
                                            textTransform: 'none',
                                    ...
                                    <IconButton
                                        className="action-button"
                                        onClick={(e) => { e.stopPropagation(); handleInterviewClick(e, candidate._id); }}
                                        sx={{
                                    ...
                                    <IconButton
                                        className="action-button"
                                        onClick={(e) => { e.stopPropagation(); handleStageClick(e, candidate._id); }}
                                        sx={{
